<script setup lang="ts">
// types
import { type Recipe } from "../types/types";

// props
defineProps<{
  recipe: Recipe;
}>();

// store
const navStore = useNavStore();
const { currentTab } = storeToRefs(navStore);
</script>

<template>
  <div class="shadow rounded-md flex flex-col">
    <NuxtImg
      sizes="xs:100vw sm:50vw lg:400px"
      format="webp"
      densities="x1"
      :src="recipe.image"
      alt=""
      class="rounded-t-md"
    />
    <div class="flex flex-col grow pt-3 pb-6 px-4 border">
      <p class="text-2xl font-semibold py-[1rem]">{{ recipe.name }}</p>
      <div
        class="font-normal w-full bg-white/80 flex gap-8 text-xl mb-4 mt-auto"
      >
        <div class="flex items-center gap-1">
          <Icon name="mdi:clock-time-eight-outline" style="color: #f79f1a" />
          <span>{{ recipe.cookTimeMinutes }}</span>
        </div>
        <div class="flex items-center gap-1">
          <Icon name="mdi:fire" style="color: #f79f1a" />
          <span>{{ recipe.caloriesPerServing }}</span>
        </div>
        <div class="flex items-center gap-1">
          <Icon name="mdi:star" style="color: #f79f1a" />
          <span>{{ recipe.rating }} ({{ recipe.reviewCount }})</span>
        </div>
      </div>

      <BaseBtn
        :to="`/recipes/${recipe.id}`"
        label="View"
        @click="currentTab = undefined"
      />
    </div>
  </div>
</template>
